...
// Adjektiv+Substantiv im Dativ Sg. nach "einem ..." — "freistehend" wird zur Adjektivform.
function toDisplayGebaeudeFormDekl(rawValue) {
  const map = {
    freistehend: 'freistehenden Gebäude',
    doppelhaus: 'Doppelhaus',
    reihenmittelhaus: 'Reihenmittelhaus',
    reihenendhaus: 'Reihenendhaus',
    mehrfamilienhaus: 'Mehrfamilienhaus'
...
// Erzeugungsart der Warmwasserbereitung.
function toDisplayWarmwasserTyp(rawValue) {
  const map = { zentral: 'zentral', durchlauferhitzer: 'Durchlauferhitzer', therme: 'Therme' };
  return map[str(rawValue).toLowerCase()] || '';
}

// Energieträger der Warmwasserbereitung (analog Heizung).
function toDisplayWarmwasserArt(rawValue) {
  const map = { gas: 'Gas', oel: 'Öl', strom: 'Strom' };
  return map[str(rawValue).toLowerCase()] || '';
}

// Kombiniert Erzeugungsart + Energieträger zu einem lesbaren Satzfragment.
// Strom → Adjektiv "elektrisch"; Gas/Öl → "X-Therme" / "X-Durchlauferhitzer"; "zentral" trägt Energieträger in Klammern.
function toDisplayWarmwasser(rawTyp, rawArt) {
  const t = str(rawTyp).toLowerCase();
  const a = str(rawArt).toLowerCase();
  const typText = toDisplayWarmwasserTyp(t);
  const artText = toDisplayWarmwasserArt(a);
  if (!typText) return artText;
  if (a === 'strom') {
    if (t === 'durchlauferhitzer') return 'elektrischer Durchlauferhitzer';
    if (t === 'therme') return 'elektrische Therme';
    if (t === 'zentral') return 'zentral (Strom)';
  }
  if (artText && (t === 'therme' || t === 'durchlauferhitzer')) {
    return `${artText}-${typText}`;
  }
  if (t === 'zentral' && artText) {
    return `zentral (${artText})`;
  }
  return typText;
}
...
module.exports = {
  ...
  toDisplayWarmwasser,
  toDisplayWarmwasserTyp,
  toDisplayWarmwasserArt,
  toDisplayFassade,
  getLandesbauordnung,
