...
function buildSachverhaltEinleitung(ctx) {
  if (!(ctx.geschossigkeitDekl || ctx.gebaeudeFormDekl || ctx.flur || ctx.flurstueck)) return '';
  ...
  const formTeil = ctx.gebaeudeFormDekl || 'Gebäude';
...
function buildWaermeschutzText(fields, ctx) {
  ...
  teile.push('Gemäß dem Gebäudebaujahr wurden an das Gebäude keine Wärmeschutzanforderungen gestellt. Dementsprechend schlecht stellt sich der Wärmeschutz im Verhältnis zu den aktuellen Wärmeschutzanforderungen dar.');
  return teile.join(' ');
}
...
function buildDocxContext(fields = {}) {
  ...
  // --- Bild-Captions ---
  // Hausansicht erscheint im Template an zwei Stellen (Bild 1 + Bild 3);
  // beide Stellen teilen sich dieselbe Eingabe.
  ...
  const warmwasserBereitstellung = T.toDisplayWarmwasser(fields.warmwasser, fields.warmwasser_art);
  ...
  const innenwandEgText = T.toDisplayInnenwand(fields.bauart_innenwand_eg);
  const innenwandUgText = T.toDisplayInnenwand(fields.bauart_innenwand_ug);
  const innenwandDickeTragend = T.toDisplayWandstaerke(fields.bauart_innenwand_dicke_tragend);
  const innenwandDickeNichttragend = T.toDisplayWandstaerke(fields.bauart_innenwand_dicke_nichttragend);
  ...
  const deckeOgText = T.toDisplayDecke(fields.bauart_decke_og) || deckeText;
  ...
  // --- Generierte Texte ---
  const sharedCtx = { geschossigkeitDekl, gebaeudeForm, gebaeudeFormDekl, flur, flurstueck, kellerungAdj,
  ...
  return {
    ...
    // Bild-Captions
    bild1_caption: bild1Caption,
    bild3_caption: bild3Caption,
    bild4_caption: bild4Caption,
    ...
    innenwand_eg: T.fallback(innenwandEgText),
    innenwand_ug: T.fallback(innenwandUgText),
    innenwand_dicke_text: T.fallback(innenwandDickeText),
